...
export default function ProjectorView() {
  ...
  const [showBoardOnly, setShowBoardOnly] = useState(false);
    ...
    <div className="min-h-screen w-full bg-indigo-950 text-white flex flex-col justify-between select-none relative overflow-hidden font-sans">
      ...
      <header className="w-full px-6 py-4 flex items-center justify-between border-b border-indigo-900/60 bg-indigo-950/80 backdrop-blur-md relative z-40">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <button
            onClick={() => setShowBoardOnly(!showBoardOnly)}
            id="btn-projector-toggle-board"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black uppercase tracking-wider bg-amber-500 hover:bg-amber-400 text-slate-950 transition-all cursor-pointer shadow-md"
          >
            <Trophy className="w-3.5 h-3.5" />
            <span>{showBoardOnly ? 'Ver Etapa' : 'Ver Tabuleiro (50)'}</span>
          </button>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-6 max-w-7xl mx-auto w-full relative z-30">
        {showBoardOnly ? (
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="p-8 sm:p-12 rounded-[48px] bg-white text-indigo-950 shadow-2xl mb-8 border-4 border-yellow-400 relative">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    <span className="text-xs sm:text-sm font-black uppercase tracking-[0.25em] text-indigo-900/60 block">
                  ...
                  <div className="flex items-center justify-center gap-4 my-3">
                    <span className="text-5xl sm:text-7xl transform -rotate-3">
              ...
              >
                <div className="p-8 sm:p-14 rounded-[48px] bg-white text-indigo-950 shadow-2xl w-full flex flex-col items-center">
                  <span className="text-xs sm:text-sm font-black uppercase tracking-widest text-indigo-900/60 mb-4">
                    {isAllTeams ? 'DINÂMICA: TODAS AS EQUIPES' : `VEZ DE ${activeTeam.name.toUpperCase()} ${activeTeam.icon}`}
                  </span>
                  ...
                  <div className="pt-8">
                    ...
                    <div className="my-6 p-4 sm:p-6 rounded-[32px] bg-yellow-400/20 border-3 border-yellow-400 max-w-md mx-auto shadow-inner">
                      ...
                      <strong className="text-4xl sm:text-6xl font-black text-amber-600 uppercase block font-mono-digits tracking-tight my-1">
                        ⭐ +{state.roundScore || 1} {(state.roundScore || 1) === 1 ? 'CASA' : 'CASAS'} NO TABULEIRO
                      </strong>
                    </div>
                    ...
                    <div className="mt-4">
                      ...
                      <p className="text-indigo-900/60 text-xs sm:text-sm font-bold uppercase tracking-wider mt-2">
                        {isAllTeams
                          ? '(Quem acertar primeiro ganha as casas no tabuleiro!)'
            ...
            {/* ================= STAGE 6: ROUND SUCCESS WITH ANIMATED PAWN BOARD ================= */}
            {state.stage === 'round_success' && (
              <motion.div
                ...
                className="text-center w-full max-w-5xl space-y-6"
              >
                <div className="p-8 sm:p-10 rounded-[48px] bg-white text-indigo-950 shadow-2xl border-4 border-emerald-500">
                  <div className="w-20 h-20 bg-emerald-500 rounded-[28px] flex items-center justify-center mx-auto mb-4 text-white shadow-xl shadow-emerald-500/40 animate-bounce">
                    <CheckCircle className="w-12 h-12 stroke-[3]" />
                  </div>

                  <h2 className="text-4xl sm:text-6xl font-black uppercase tracking-tight text-emerald-600 mb-2">
                    🎉 ACERTOU! PEÃO AVANÇOU!
                  </h2>

                  <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl bg-yellow-400 border-4 border-yellow-300 shadow-xl my-2">
                    <Star className="w-6 h-6 fill-indigo-950 stroke-none" />
                    <span className="text-2xl sm:text-4xl font-black text-indigo-950 uppercase font-mono-digits">
            ...
            {state.stage === 'match_summary' && (
              <motion.div
                ...
                className="text-center w-full max-w-5xl space-y-6"
              >
                <div className="p-8 sm:p-12 rounded-[48px] bg-white text-indigo-950 shadow-2xl border-4 border-yellow-400">
                  <div className="w-24 h-24 bg-yellow-400 rounded-3xl flex items-center justify-center mx-auto mb-4 text-indigo-950 shadow-2xl border-4 border-yellow-300 transform -rotate-3">
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs sm:text-sm font-black uppercase tracking-wider text-indigo-200 whitespace-nowrap">
              TABULEIRO (META {boardLength} CASAS):
            </span>
          </div>
