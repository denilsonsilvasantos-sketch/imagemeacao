    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-4 md:py-6 select-none space-y-6" id="game-board-view">
      ...
      <div className="w-full bg-indigo-900/60 backdrop-blur-md border-2 border-indigo-800/80 rounded-[32px] p-4 sm:p-5 shadow-2xl">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          {/* Quick Action Buttons, Screen 2 Controller & Board Toggle */}
          <div className="flex items-center gap-2 flex-wrap justify-end w-full md:w-auto">
            {/* Screen 2 Layout Selector for Operator */}
            <div className="flex items-center bg-indigo-950/80 p-1 rounded-2xl border border-indigo-700/60 shadow-inner">
              <span className="text-[10px] font-black uppercase text-indigo-300 px-2 flex items-center gap-1">
                <Tv className="w-3 h-3 text-amber-400" />
                <span className="hidden sm:inline">Tela 2:</span>
              </span>
              <button
                onClick={() => handleSetProjectorLayout('split')}
                id="btn-op-layout-split"
                className={`px-2.5 py-1 rounded-xl text-[11px] font-black uppercase transition-all cursor-pointer ${
                  projectorLayout === 'split'
                    ? 'bg-amber-400 text-slate-950 shadow font-black'
                    : 'text-indigo-200 hover:text-white'
                }`}
                title="Exibir no Projetor: Dividido (Palco + Caminho do Tabuleiro)"
              >
                Dividido
              </button>
              <button
                onClick={() => handleSetProjectorLayout('path')}
                id="btn-op-layout-path"
                className={`px-2.5 py-1 rounded-xl text-[11px] font-black uppercase transition-all cursor-pointer ${
                  projectorLayout === 'path'
                    ? 'bg-amber-400 text-slate-950 shadow font-black'
                    : 'text-indigo-200 hover:text-white'
                }`}
                title="Exibir no Projetor: Caminho do Tabuleiro em tela cheia"
              >
                Caminho
              </button>
              <button
                onClick={() => handleSetProjectorLayout('stage')}
                id="btn-op-layout-stage"
                className={`px-2.5 py-1 rounded-xl text-[11px] font-black uppercase transition-all cursor-pointer ${
                  projectorLayout === 'stage'
                    ? 'bg-amber-400 text-slate-950 shadow font-black'
                    : 'text-indigo-200 hover:text-white'
                }`}
                title="Exibir no Projetor: Foco no Palco/Timer"
              >
                Palco
              </button>
            </div>
            ...
            <button
              ...
              className="px-3 py-2 rounded-2xl bg-indigo-800 hover:bg-indigo-700 text-yellow-300 font-black text-xs uppercase tracking-wider border border-indigo-600 transition-all cursor-pointer flex items-center gap-1.5 shadow-md"
              title="Abrir a 2ª Tela / Janela do Projetor"
            >
              <Tv className="w-3.5 h-3.5 text-yellow-400" />
              <span className="hidden lg:inline">Abrir</span> Tela 2
            </button>

            <button
              onClick={() => setShowBoardModal(!showBoardModal)}
              id="btn-toggle-board-view"
              className="px-3.5 py-2 rounded-2xl bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-black text-xs uppercase tracking-wider shadow-lg shadow-amber-500/20 border-2 border-yellow-300 transition-all cursor-pointer flex items-center gap-1.5"
            >
              <Trophy className="w-3.5 h-3.5" />
              <span>{showBoardModal ? 'Ocultar Trilha' : 'Ver Trilha (50)'}</span>
            </button>
            ...
            <button
              ...
              id="btn-finish-match"
              className="px-3 py-2 rounded-2xl bg-white/10 hover:bg-white/20 text-indigo-200 hover:text-white text-xs font-black uppercase tracking-wider border border-white/20 transition-all cursor-pointer whitespace-nowrap flex items-center gap-1.5 backdrop-blur-md"
